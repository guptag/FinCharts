...
function fetchData(cookieData, chartKeys) {
  ...
  var duration =
    chartKeys.duration === "daily"
      ? "d"
      : chartKeys.duration === "weekly"
      ? "w"
      : "m";
  ...
  getFromCache(cacheKey).then(function (rowData /* {key: "", data: ""} */) {
    if (rowData) {
      fetchDeferred.resolve({ priceData: rowData.data, ticker: ticker });
    } else {
      getDataFromServer(dataUrl, { cookie: cookieData.cookie }).then(function (
        priceData
      ) {
        addToCache(cacheKey, priceData).then(function () {
...
function parseData(data) {
  ...
  _.each(dateRecords, function (dataRow) {
    ...
    if (data.low < returnData.min) returnData.min = data.low;

    if (data.high > returnData.max) returnData.max = data.high;

    if (data.volume < returnData.minVolume) returnData.minVolume = data.volume;

    if (data.volume > returnData.maxVolume) returnData.maxVolume = data.volume;
  });
